...
static HANDLE create_device_handle(mb_disc_private* disc, const char* device)
{
	...
	const char* colon;
	size_t len;
	filename[0] = '\0';  filename[1] = '\0'; filename[2] = '\0'; filename[3] = '\0';
	//strcpy_s(filename, 255, "\\\\.\\");
	wcscpy_s(filename, L"\\\\.\\");
	len = strlen(device);
	colon = strchr(device, ':');
	if (colon) {
	...
	//strncat_s(filename, 255, device, len > 120 ? 120 : len);
	const size_t cSize = strlen(device) + 1;
	wchar_t* wc = new wchar_t[cSize];

	size_t p = 0;
	errno_t  result = mbstowcs_s(&p, wc, cSize, device, _TRUNCATE);
	wcsncat_s(filename, 255, wc, len > 120 ? 120 : len);
	//wcsncat_s(filename, 255, device, len > 120 ? 120 : len);

	hDevice = CreateFile((LPCWSTR)filename, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, 0, NULL);

	if (hDevice == INVALID_HANDLE_VALUE) {
		snprintf(disc->error_msg, MB_ERROR_MSG_LENGTH + 32, "cannot open the CD audio device '%s'", device);
		return 0;
	}
	return hDevice;
}
...
static void read_disc_mcn(HANDLE hDevice, mb_disc_private* disc)
{
	...
	if (bResult == FALSE) {
		...
		strncpy_s(disc->mcn, strlen(disc->mcn) + MCN_STR_LENGTH + 32, (char*)data.MediaCatalog.MediaCatalog, MCN_STR_LENGTH);
	}
}
...
static void read_disc_isrc(HANDLE hDevice, mb_disc_private* disc, int track)
{
	...
	if (bResult == FALSE) {
		...
		strncpy_s(disc->isrc[track], strlen(disc->isrc[track]) + ISRC_STR_LENGTH + 16, (char*)data.TrackIsrc.TrackIsrc, ISRC_STR_LENGTH);
	}
}
...
int get_nth_device(int number, char* device, int device_length) {
	...
	for (i = 0; i <= 25; i++) {
		if (mask >> i & 1) {
			...
			if (GetDriveType((LPCWSTR)tmpDevice) == DRIVE_CDROM) {
				counter++;

				if (counter == number)
				{
					strncpy_s(device, strlen(device) + device_length, tmpDevice, device_length);
					return TRUE;
				}
			}
		}
...
char* mb_disc_get_default_device_unportable(void) {
	...
	{
		return (char*)MB_DEFAULT_DEVICE;
	}
	return default_device;
}

int __cdecl mb_disc_has_feature_unportable(enum discid_feature feature) {
	switch (feature) {
...
int mb_disc_winnt_read_toc(HANDLE device, mb_disc_private* disc, mb_disc_toc* toc)
{
	...
	bResult = DeviceIoControl(device, IOCTL_CDROM_READ_TOC, NULL, 0, &cd, sizeof(cd), &dwReturned, NULL);
	if (bResult == FALSE) {
...
int __cdecl mb_disc_read_unportable(mb_disc_private* disc, const char* device, unsigned int features)
{
	mb_disc_toc toc;
	char tmpDevice[MAX_DEV_LEN];
	HANDLE hDevice;
