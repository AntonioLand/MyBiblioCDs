...
int mb_disc_load_toc(mb_disc_private* disc, mb_disc_toc* toc) {
	...
	{
		_snprintf_s(disc->error_msg, MB_ERROR_MSG_LENGTH, MB_ERROR_MSG_LENGTH, "invalid CD TOC - first track number must be 1 or higher");
		return 0;
	}

	if (toc->last_track_num < 1) {
		_snprintf_s(disc->error_msg, MB_ERROR_MSG_LENGTH, MB_ERROR_MSG_LENGTH, "invalid CD TOC - last track number must be 99 or lower");
		return 0;
	}
	// printf("toc->last_track_num %d\n", toc->last_track_num);
	...
	for (i = toc->first_track_num; i <= toc->last_track_num; i++) {
		...
		//printf("i=%i\t &toc->tracks[i]=%i\t toc->tracks[i]=%i\tlast_audio_track = %i \n", i,&toc->tracks[i],toc->tracks[i],last_audio_track);
	}

	if (last_audio_track < 0)
	{
		_snprintf_s(disc->error_msg, MB_ERROR_MSG_LENGTH, MB_ERROR_MSG_LENGTH, "no actual audio tracks on disc: CDROM or DVD?");
		return 0;
	}
	...
	// printf("disc->first_track_num = %i\tdisc->last_track_num= %i\n", disc->first_track_num, disc->last_track_num);
	...
	for (i = first_audio_track; i <= last_audio_track; i++) {
		...
		// printf("track = %d\tdisc->track_offsets[i] = %d, track->address = %d\n", track, disc->track_offsets[i], track->address);
	}
	...
	//   printf("------------------------YYYYY-------------------------\n");
	//   printf("track = %i\tdisc->track_offsets[0] = %i,\ttrack->address = %i\n", track, disc->track_offsets[0], track->address);
